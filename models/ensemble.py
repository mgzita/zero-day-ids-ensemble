...
models/ensemble.py
==================
Ensemble fusion of AE + VAE + Isolation Forest.

Fusion weights (paper section 4.6):
    0.4 * AE_recon + 0.4 * VAE_recon + 0.2 * IF

Score normalisation: p1/p99 percentile normalisation fitted on
...
class EnsembleDetector:
    """
    Weighted ensemble of AE + VAE + IsolationForest.
    ...
    def fit_normaliser(self, X_val: np.ndarray) -> "EnsembleDetector":
        ...
        for name, scores in raw.items():
            ...
            log.info("  %s scores: p1=%.4f  p99=%.4f", name, s_min, s_max)
        return self
    ...
    def ensemble_scores(self, X: np.ndarray) -> np.ndarray:
        ...
        fused = (
            self.WEIGHTS["ae"]  * normed["ae"]  +
            self.WEIGHTS["vae"] * normed["vae"] +
    ...
    def _raw_scores(self, X: np.ndarray) -> Dict[str, np.ndarray]:
        return {
            "ae":  self.ae.anomaly_scores(X),
            "vae": self.vae.anomaly_scores(X),
